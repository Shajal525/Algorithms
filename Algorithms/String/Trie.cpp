...
public:
    ...
    void deletee(string st);
    bool delete_helper(struct tree *rt, string st, int level, int len);
...
bool TRIE::delete_helper(struct tree *rt, string st, int level, int len)
{
    if(rt == NULL)
        return false;
    if(level == len){
        ...
        int in = st[level] - 'a';
        if(delete_helper(rt->chiel[in], st, level+1, len)){
            delete(rt->chiel[in]);
            rt->chiel[in] = NULL;
            return (rt->is_leaf && is_it_free(rt));
        }
    }
return false;
}

void TRIE::deletee(string st)
{
    delete_helper(root, st, 0, st.length());
return;
}
...
void TRIE::input_fun()
{
    ...
    cout << searchh(root, "shajalil") << endl;
    cout << searchh(root, "shushmoy") << endl;
    cout << searchh(root, "shutup") << endl;
